...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Mock API integration for Pathao
    // In a real application, you would make an API request to Pathao's endpoints here.
    
    // Simulate API request delay
    await new Promise((resolve) => setTimeout(resolve, 1000));

    const consId = `PATHAO-${order.id}-${Math.floor(Math.random() * 10000)}`;
    const trackCode = `TRK-PT-${order.id}`;
